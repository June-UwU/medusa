...
#define write_to_console(format, ...)      \
  if (level >= log_config.console_level) { \
    printf(format, __VA_ARGS__);           \
  }
#define write_to_file(file, format, ...)                       \
...
void write_log(medusa::log_level level, const char* format, ...) {
  ...
  write_to_console("%s%s\x1b[0m", get_color_code(level), buffer);
  write_to_file(log_file, "%s", buffer);
}
...
void write_timed_log(medusa::log_level level, const char* format, ...) {
  ...
  write_to_console("\033[1;32m[%lf] %s%s\x1b[0m", get_elapsed_seconds(),
                   get_color_code(level), buffer);
  write_to_file(log_file, "%s", buffer);
}
